...
└────────────────────────────────┬───────────────────────────┘
                ...
                └────┬───────────────┬───────────────┬┘
                     │               │               │
              ┌──────▼──────┐ ┌──────▼──────┐ ┌──────▼─────────────┐
              │ Neon Postgres│ │ Vercel Blob │ │  Markdown render   │
...
The demo is single-tenant. One connector deployment, one user, one database namespace. Multi-user identity is documented under infrastructure proposals, not built.
...
- **Neon Postgres** — the system of record for entries. Provisioned through the Vercel Marketplace (Vercel's relational storage partner). One `entries` table with indexes on `(namespace, created_at DESC)` and `(namespace, filing_year)` for the common queries. Each row holds a `transcript_blob_key` pointing into Vercel Blob.
- **Vercel Blob** — durable object storage for the full source conversation transcript that accompanies each entry. Transcripts are written here so the user can return to the unedited thread later, independent of claude.ai retention. Postgres holds the entry's curated reasoning; Blob holds the raw record. User must consent to saving full record.
...
- **Synthetic seed entries (`/app/data/seed_entries.json`)** — a small set of pre-populated entries that demonstrate retrieval against multi-year reasoning. Loaded into Postgres on first deploy via a one-shot init route that also runs `CREATE TABLE IF NOT EXISTS`. Each seed entry pairs with a synthetic transcript stored in Blob.
...
2. **Lookup.** The connector runs an `ILIKE` substring match over entry decisions, rationales, and alternatives and returns matches ordered by most recent first.
...
Validated on write. Missing required fields, empty rationale, or empty decision is rejected before the Postgres write. `transcript_blob_key` is optional: the user gives a separate consent for the transcript after confirming the entry, and may decline.
...
- **Vercel** — hosting for the connector (serverless functions) and storage. Needs a Vercel project with the Neon Postgres and Vercel Blob marketplace integrations enabled.
- **Neon Postgres** — system of record for entries. Free tier is sufficient for demo scale.
...
| `HANDBOOK_NAMESPACE` | `default` | row-level namespace (stored in the `namespace` column) and Blob key prefix; lets one database back multiple deployments without collision |
...
- **Schema violations.** Every write goes through a validator that enforces required fields (`decision`, `rationale`, `filing_year`) and types. Malformed payloads are rejected before touching Postgres. Transcript URL and transcript blob are optional.
- **Orphaned entries.** Save flow writes the transcript to Blob first, then the entry row to Postgres. A Postgres failure leaves a harmless orphan blob; a Blob failure prevents the Postgres write entirely. There is no state where an entry references a transcript that does not exist.
...
- **Divergent copies.** The user edits the CoWork markdown file while the connector store still holds the old version, or vice versa.
  - *Mitigation:* two-way sync is out of scope for v1. The Postgres database is the system of record; the CoWork file is a snapshot.
...
│   │   ├── db.ts                   # Neon Postgres client (entry CRUD + schema bootstrap)
...
- **Multi-user identity.** The demo is single-tenant. A real Handbook would scope entries to a user. Cleanest path: the MCP connector reads a user id from the connector install context (each user's Claude install would surface a stable identifier to the connector), then entries are scoped by a `user_id` column. No login flow on the user's side.
- **Two-way sync between Postgres and CoWork.** Today the markdown file is a snapshot. In production, edits in CoWork should flow back to Postgres (or Postgres should be derived from CoWork files, inverting the system of record). Conflict resolution becomes real.
- **Schema evolution.** When the entry schema changes (e.g., adding a `tags` field), existing entries need a migration path. Standard SQL migrations (`ALTER TABLE`) handle this cleanly, but they need to be tracked and run in order.
